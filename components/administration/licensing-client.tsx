...
export function LicensingClient({ hasVBR, hasVB365 }: LicensingClientProps) {
    // Determine default tab based on what's configured
    const defaultTab = hasVBR ? "vbr" : hasVB365 ? "vb365" : "none"
    const [activeTab, setActiveTab] = useState<"vbr" | "vb365" | "none">(defaultTab)

    // VBR State
    const [license, setLicense] = useState<LicenseModel | null>(null)
    const [loading, setLoading] = useState(hasVBR)
    ...
    const fetchVBRLicense = useCallback(async () => {
        if (!hasVBR) return
        try {
            setLoading(true)
    ...
    const fetchVB365Data = useCallback(async () => {
        if (!hasVB365) return
        try {
    ...
    // Only fetch VBR license on mount if VBR is configured
    useEffect(() => {
        if (hasVBR) {
            fetchVBRLicense()
        }
    }, [hasVBR, fetchVBRLicense])

    // Fetch VB365 data when tab switches to vb365
    useEffect(() => {
        if (activeTab === "vb365" && hasVB365) {
            fetchVB365Data()
        }
    }, [activeTab, hasVB365, fetchVB365Data])
    ...
    const handleRefresh = () => {
        if (activeTab === "vbr" && hasVBR) {
            fetchVBRLicense()
        } else if (activeTab === "vb365" && hasVB365) {
            fetchVB365Data()
        }
    }
            ...
            <div className="flex-1 overflow-auto">
                <div className="container mx-auto py-8 px-4 space-y-4">
                    ...
                    <Alert>
                        ...
                        <AlertDescription>
                            Configure VEEAM_API_URL or VBM_API_URL environment variables to view licensing information.
                        </AlertDescription>
                    </Alert>
                </div>
            </div>
    ...
    if (loading && !license && hasVBR && activeTab === "vbr") {
            ...
            <div className="container mx-auto py-8 px-4 space-y-4">
                ...
                <div className="flex flex-col sm:flex-row gap-4 items-start sm:items-center justify-between">
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button variant="outline" size="sm" onClick={handleCreateReport} disabled={generatingReport || activeTab === "none"}>
                ...
                <Tabs value={activeTab} onValueChange={(v) => setActiveTab(v as "vbr" | "vb365")} className="space-y-4">
                    <TabsList>
                        {hasVBR && <TabsTrigger value="vbr">Veeam Backup & Replication</TabsTrigger>}
                        {hasVB365 && <TabsTrigger value="vb365">Veeam Backup for Microsoft 365</TabsTrigger>}
                    </TabsList>
